...
const SearchTaskDetails = () => {
  ...
  const query = new URLSearchParams(location.search); // Create URLSearchParams to read query parameters
  const title = query.get('title'); // Extract title from query parameters
  ...
  }, [id,modalOpen,modalEditOpen]);

  const fetchtask = async () => {
    const taskData = await axiosInstance.get(`/task/task-details/${id}`);
...
export default React.memo(SearchTaskDetails);
